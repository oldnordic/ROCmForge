//! Individual operation execution functions for HIP backend.
//!
//! This module contains the implementation of each GGML IR operation.
//! Each function validates inputs/outputs and delegates to the appropriate
...
impl super::HipGgmlBackend {
    /// Execute GetRows operation - extracts rows from weights based on token IDs.
    pub(crate) fn execute_get_rows(&self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
    ...
    /// Execute MatMul operation - matrix multiplication.
    pub(crate) fn execute_matmul(&self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
        if inputs.len() != 2 || outputs.len() != 1 {
            return Err(GgmlError::InvalidShape(
    ...
    /// Execute Add operation - element-wise addition.
    pub(crate) fn execute_add(&self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
        if inputs.len() != 2 || outputs.len() != 1 {
            return Err(GgmlError::InvalidShape(
    ...
    /// Execute Scale operation - element-wise scaling.
    pub(crate) fn execute_scale(&self, inputs: &[TensorId], outputs: &[TensorId], factor: f32) -> GgmlResult<()> {
    ...
    pub(crate) fn execute_layernorm(&mut self, inputs: &[TensorId], outputs: &[TensorId], eps: f32) -> GgmlResult<()> {
        ...
        let input_shape = TensorShape::from_dims(&input_desc.shape);
        let weight_shape = TensorShape::from_dims(&weight_desc.shape);
        let output_shape = TensorShape::from_dims(&output_desc.shape);

        let input_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            input_buf.clone(),
            input_shape,
        )
        .map_err(|e| GgmlError::Backend(e.to_string()))?;
        let weight_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            weight_buf.clone(),
            weight_shape,
        )
        .map_err(|e| GgmlError::Backend(e.to_string()))?;
        let mut output_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            output_buf.clone(),
            output_shape,
        )
        ...
        let bias_tensor = if inputs.len() == 3 {
            ...
            let bias_shape = TensorShape::from_dims(&bias_desc.shape);
            Some(
                DeviceTensor::from_buffer(
                    self.hip_backend(),
                    bias_buf.clone(),
                    bias_shape,
                )
    ...
    pub(crate) fn execute_rmsnorm(&self, inputs: &[TensorId], outputs: &[TensorId], eps: f32) -> GgmlResult<()> {
        if inputs.len() != 2 || outputs.len() != 1 {
            return Err(GgmlError::InvalidShape(
    ...
    /// Execute RoPE (Rotary Position Embedding) operation.
    pub(crate) fn execute_rope(&self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
    ...
    pub(crate) fn execute_softmax(&self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
        if inputs.len() != 1 || outputs.len() != 1 {
            return Err(GgmlError::InvalidShape(
    ...
    pub(crate) fn execute_attention(&mut self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
        ...
        let q_shape = TensorShape::from_dims(&q_desc.shape);
        let k_shape = TensorShape::from_dims(&k_desc.shape);
        let v_shape = TensorShape::from_dims(&v_desc.shape);
        let scores_shape = TensorShape::from_dims(&scores_desc.shape);
        let softmax_shape = TensorShape::from_dims(&softmax_desc.shape);
        let out_shape = TensorShape::from_dims(&output_desc.shape);

        let q_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            q_buf.clone(),
            q_shape,
        )
        .map_err(|e| GgmlError::Backend(e.to_string()))?;
        let k_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            k_buf.clone(),
            k_shape,
        )
        .map_err(|e| GgmlError::Backend(e.to_string()))?;
        let v_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            v_buf.clone(),
            v_shape,
        )
        .map_err(|e| GgmlError::Backend(e.to_string()))?;
        let mut scores_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            scores_buf.clone(),
            scores_shape,
        )
        .map_err(|e| GgmlError::Backend(e.to_string()))?;
        let softmax_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            softmax_buf.clone(),
            softmax_shape,
        )
        .map_err(|e| GgmlError::Backend(e.to_string()))?;
        let mut out_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            out_buf.clone(),
            out_shape,
        )
    ...
    pub(crate) fn execute_mask(&self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
        if inputs.len() != 2 || outputs.len() != 1 {
            return Err(GgmlError::InvalidShape(
    ...
    pub(crate) fn execute_swiglu(&self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
        if inputs.len() != 2 || outputs.len() != 1 {
            return Err(GgmlError::InvalidShape(
    ...
    /// Execute MLP with SwiGlu activation.
    pub(crate) fn execute_mlp_swiglu(&mut self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
        ...
        .map_err(|e| GgmlError::Backend(e.to_string()))?;
        let gate_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            gate_buf.clone(),
        ...
        .map_err(|e| GgmlError::Backend(e.to_string()))?;
        let up_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            up_buf.clone(),
            TensorShape::from_dims(
        ...
        .map_err(|e| GgmlError::Backend(e.to_string()))?;
        let down_tensor = DeviceTensor::from_buffer(
            self.hip_backend(),
            down_buf.clone(),
            TensorShape::from_dims(
    ...
    /// Execute SplitQKV operation - split combined QKV tensor into separate Q, K, V.
    pub(crate) fn execute_split_qkv(&self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
    ...
    /// Execute View/Reshape operation - creates a view into an existing buffer.
    pub(crate) fn execute_view_reshape(&mut self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
        if inputs.len() != 1 || outputs.len() != 1 {
            return Err(GgmlError::InvalidShape(
    ...
    /// Execute Copy operation - copies data from one buffer to another.
    pub(crate) fn execute_copy(&self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
        if inputs.len() != 1 || outputs.len() != 1 {
            return Err(GgmlError::InvalidShape(
    ...
    /// Execute quantized MatMul Q4_0 operation.
    pub(crate) fn execute_matmul_q4_0(&self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
        if inputs.len() != 2 || outputs.len() != 1 {
            return Err(GgmlError::InvalidShape(
    ...
    /// Execute quantized MatMul Q8_0 operation.
    pub(crate) fn execute_matmul_q8_0(&self, inputs: &[TensorId], outputs: &[TensorId]) -> GgmlResult<()> {
        if inputs.len() != 2 || outputs.len() != 1 {
            return Err(GgmlError::InvalidShape(
    ...
    /// Execute Accumulate operation - accumulates values with offset.
    pub(crate) fn execute_accumulate(&self, inputs: &[TensorId], outputs: &[TensorId], offset: usize) -> GgmlResult<()> {
        if inputs.len() != 2 || outputs.len() != 1 {
            return Err(GgmlError::InvalidShape(
