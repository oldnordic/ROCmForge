...
fn get_or_init_q4_0_cache() -> Result<&'static Mutex<Option<Q4_0KernelCache>>, HipError> {
    ...
    let kernel_path = option_env!("Q4_0_MATMUL_HSACO")
        .ok_or_else(|| HipError::KernelLoadFailed("Q4_0_MATMUL_HSACO not set at compile time. Rebuild the project.".to_string()))?;

    if !Path::new(&kernel_path).exists() {
        return Err(HipError::KernelLoadFailed(format!(
            "Q4_0 matmul HSACO file not found at {} (compiled path from build.rs)",
            kernel_path
        )));
    }
