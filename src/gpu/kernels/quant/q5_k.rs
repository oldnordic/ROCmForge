//! Q5_K quantization kernel wrappers.
//!
...
mod tests {
    ...
    #[test]
    fn quantize_q5_k_rejects_zero_n() {
        let result = quantize_q5_k(std::ptr::null(), std::ptr::null_mut(), 0);
        assert!(result.is_err());
    }

    #[test]
    fn dequantize_q5_k_rejects_zero_n() {
        let result = dequantize_q5_k(std::ptr::null(), std::ptr::null_mut(), 0);
        assert!(result.is_err());
    }

    #[test]
    fn dequantize_q5_k_batched_rejects_zero_batch() {
        let result = dequantize_q5_k_batched(std::ptr::null(), std::ptr::null_mut(), 256, 0);
        assert!(result.is_err());
    }

    #[test]
    fn verify_q5_k_accuracy_rejects_zero_n() {
        let result =
            verify_q5_k_accuracy(std::ptr::null(), std::ptr::null(), std::ptr::null_mut(), 0);
        assert!(result.is_err());
    }

    #[test]
    fn finalize_q5_k_metrics_rejects_zero_n() {
        let result = finalize_q5_k_metrics(std::ptr::null(), std::ptr::null_mut(), 0);
        assert!(result.is_err());
    }
}
