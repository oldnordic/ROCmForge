...
### Features
- **Gemma4 GPU Support (Alternative Attention + PLE)** — Added full GPU inference support for Gemma4 models with hybrid attention architecture. Gemma4 uses alternative attention in layers 5,11,17,23,29,35,41,47 (omitting attn_v tensors and reusing K projection as V) with kv_head_dim=256 vs global attn_head_dim=512. Implemented per-layer head dimension handling, optional PLE (Per-Layer Embedding) support, and K→V copy for alternative attention layers. Gemma4 12B Q4_0 inference achieves 23-24 tok/s decode on AMD RX 7900 XT (RDNA3) with 12.3GB VRAM usage.
- **Gemma4 PLE VRAM Optimization** — Fixed VRAM allocation inefficiency for Gemma4 Per-Layer Embedding (PLE) tensors. Changed from per-layer duplication (35 layers × 3 PLE buffers = 105 allocations, ~1540 MB waste) to shared model-level buffers. Layer loaders now skip PLE duplication and forward pass uses shared model-level PLE when layer-level is None. Reduces VRAM overhead by ~35× for Gemma4 models, enabling E2B variants to fit in available GPU memory. Other models unaffected (PLE gates on `architecture == "gemma4"`).
- **Per-Layer Head Dimension Discipline** — Enforced strict `config.head_dim_for_layer(layer_idx)` and `config.kv_head_dim_for_layer(layer_idx)` usage throughout GPU codebase to prevent dimension mismatches that caused GPU memory access faults. Replaced all direct `config.head_dim` field access with per-layer accessor methods in attention operations, RoPE, RMS norm, and buffer sizing calculations.
- **Gemma4-Specific GPU Buffer Sizing** — Modified `GpuForwardScratch::new` to use maximum per-layer q_size/kv_size for Gemma4 instead of global constants. Prevents buffer allocation mismatches for hybrid attention architectures where different layers use different head dimensions.
- **Single-Row Q4_0 GEMV Investigation (RDNA3)** — Benchmarked single-row vs 8-subwave multi-row Q4_0 GEMV geometry on Qwen2.5-7B-Instruct-Q4_0 (hidden=3584, the bandwidth-bound regime). Result: rocmforge's single-row kernel is consistently ~2% SLOWER than 8-subwave (50.3 vs 51.6 tok/s, 6 runs, graph on and off). This CONTRADICTS hipfire's measurement where single-row is optimal on RDNA3 — rocmforge's single-row kernel has a quality gap (under investigation: inner-loop dequant pattern, input-vector caching). The 8-subwave multi-row path remains the default; single-row stays opt-in via `ROCMFORGE_Q4_0_SINGLE_ROW=1`. The profiling infrastructure and benchmark harness are now in place to validate future kernel improvements against the real 7B target.
- **DP4A Q4_0/Q8_0 Decode Kernels** — Added int8 dot-product acceleration for Q4_0 × Q8_0 GEMV, gate-up, and SwiGLU kernels via `__ockl_sdot4` on gfx1030/gfx1100/gfx1101/gfx1102, with a software fallback on older architectures. `ROCMFORGE_Q4_0_Q8_DP4A=1` opt-in enables the DP4A fused SwiGLU dispatch path in `src/gpu/ops/gate_up.rs`; default decode keeps the existing scalar fastpaths. `ROCMFORGE_Q4_0_Q8_SINGLE_ROW=1` switches to the single-row high-occupancy launch configuration. Bit-level packing uses unsigned nibble extraction to stay numerically equivalent to the scalar path.
- **Automatic DP4A Kernel Selection by Model Size** — Replaced manual `ROCMFORGE_Q4_0_Q8_DP4A` environment variable with automatic model-size detection. Added `ModelConfig::parameter_count_billion()` and `ModelConfig::should_use_dp4a()` methods. Models >3B parameters (e.g., Qwen2.5-7B) automatically use DP4A kernels for 1.5-2× speedup; models ≤3B (e.g., Qwen2.5-0.5B) use scalar kernels for numerical correctness. All DP4A dispatch locations updated: `gate_up.rs`, `qkv.rs`, `forward_graph.rs`, `shortconv.rs`, `prefill_layer.rs`. Manual env var override still supported. Small model correctness tests now pass with scalar kernels (previously failed with 3× numerical error from DP4A wave reduction).
- **High-Occupancy Fused Attention Prefill** — Replaced the single-head, thread-serial prefill attention kernel with a wavefront-parallel multi-head kernel (`flash_attn_prefill_strided_multi_head_kernel`) using a `dim3(seq_len, num_heads)` grid. Each block cooperatively scans the causal key range `0..=query_pos` and over `head_dim`, preserving KV-lora reconstruction and causal masking semantics.
- **Decode Graph Health Observability** — Added non-intrusive atomic telemetry for HIP decode graphs (`src/gpu/decode_graph_health.rs`). Tracks captures, replays, cache hits/misses, cache updates, fallbacks, and runtime disables. Enable with `ROCMFORGE_OBSERVE_DECODE_GRAPH_HEALTH=1`; query via `GpuDecodeGraphHealthSnapshot` / `decode_graph_health_snapshot()`.
- **ROCm Grounded Coding Skill** — Adapted the core grounded coding skill to AMD ROCm, mandating VRAM budgeting, sequential testing (`--test-threads=1`), and hardware safety protocols to prevent GPU resets.
- **X-Ray Analysis for 7B Models** — Fixed `run_kappa_experiment.py` to properly support GPU-generated traces which lack intermediate node records for prompt prefixes. Captured 80 traces for Qwen2.5-7B and confirmed the "hourglass" attention release pattern persists at larger scale.
- **Critical GPU Kernel Safety Fixes** — Identified and fixed unaligned memory loads in `q4_0_gemv.hip` and `batched_q4_0.hip` that were causing `NaN` logits and potential GPU crashes on RDNA3 (gfx1100) hardware. Switched unaligned `reinterpret_cast<uint32_t*>` to safe `__builtin_memcpy` and applied `__attribute__((aligned(16)))` to shared memory buffers.
- **VRAM Budgeting Enforced** — Integrated `rocm-smi` and `rocmforge --gpu` preflight checks into the development workflow to prevent VRAM over-allocation and system freezes.

### Tests
- **Gemma4 GPU Tests** — Added Gemma4-specific test coverage: GPU vs CPU divergence tests for alternative attention layers (5,11,17,23,29,35,41,47), PLE tensor loading validation, per-layer head dimension correctness, K→V copy operations. Gemma4 12B Q4_0 real-model decode tests verify 23-24 tok/s decode throughput. Tests use gating by `ROCMFORGE_TEST_MODELS_GEMMA4` path environment variable.
- **DP4A Q4_0/Q8_0 Numerical Verification** — `cargo test --features gpu --release --test gpu_q4_0_q8_dispatch -- --test-threads=1` passes with `ROCMFORGE_Q4_0_Q8_DP4A=1`, matching the scalar CPU/GPU reference within tolerance.
- **Multi-Head Prefill Correctness** — `tests/integration_gpu.rs::test_flash_attn_prefill_strided_kernel_correctness` now exercises the new multi-head wrapper once per batch and compares against a CPU oracle.
- **Decode Graph Health Unit Tests** — Added `src/gpu/decode_graph_health.rs` doctests/unit tests covering snapshot, reset, and record helpers under `--lib decode_graph_health`.
- **DP4A Decode Benchmark** — Added `benches/gpu_decode_dp4a_vs_baseline.rs` with a `Cargo.toml` bench entry. Compares real-model greedy decode throughput with `ROCMFORGE_Q4_0_Q8_DP4A=0` vs `1`, disabling decode graph so the measurement reflects raw kernel dispatch. On a merged Qwen2.5-7B-Instruct-Q4_0 GGUF, measured **78.78 tok/s** with DP4A vs **50.26 tok/s** baseline, a **1.57× decode speedup**.

### TODO
- **Performance Tuning** — Aim for 200 tok/s on 7B models by fully optimizing the GEMV and fused gate-up kernels for RDNA3 wave sizes (wave32).
- **Harness Cleanup** — Clean up temporary experiment scripts (`run_kappa_experiment_v*.py`) and finalize ROCm skill documentation.

### Fixed
- **Gemma4 GPU Memory Access Violations** — Fixed 4 critical memory access violations in alternative attention layers causing GPU page faults. Fixed K→V memcpy size computation (scratch.k.size() → actual kv_size), RMS norm using wrong head dimension (attn_head_dim → kv_head_dim), RoPE using wrong dimension, and prefill buffer sizing. All changes enforce per-layer head dimension discipline using config.head_dim_for_layer() and config.kv_head_dim_for_layer() instead of global constants.
- **Gemma4 E2B Runtime Crash** — Gemma4 E2B loads successfully with optimized PLE VRAM allocation (4.3 GB, 35× improvement) but crashes during decode execution. Memory access fault occurs in forward pass (not PLE allocation - that's fixed). Crash unrelated to PLE optimization; requires separate investigation of Gemma4-specific forward pass logic (alternative attention layers 5,11,17,23,29,35,41,47 or head dimension mismatches). Other models unaffected.
- **GPU Decode NaN / Q8 Fastpath Layout Mismatch** — Fixed the streamed Q8 activation workspace format to write standard 34-byte `Q8_0` blocks with `f16` scales, aligned the decode fastpath workspace sizing with that format, and restored finite LM-head decode on GPU. Real-model greedy decode now matches the CPU token path again.
- **Q4_1/Q4_K/Q5_K Integration Oracles** — Replaced stale or circular GPU integration references with real CPU quantized GEMV or dequantized row oracles. Implemented the missing CPU `gemv_q4_1_transposed` path in `src/`, corrected unsupported in-place residual test assumptions, and brought the serial GPU integration suite back to meaningful end-to-end validation.
- **Quantized GEMV Validation Coverage** — Updated the older `quant_integration` Q4_K/Q5_K synthetic checks to use quant-type-specific quantizers plus scale-aware tolerances, so they validate the same packed formats and numerics exercised by the newer real-model parity suite instead of failing on stale assumptions.

### Tests
- **Full Serial ROCm GPU Gate** — The full `cargo test --features gpu -- --test-threads=1` suite now passes again after the oracle and quant-validation cleanup, including `integration_gpu`, `quant_integration`, real-model decode parity, Q4_K_M layer-0 parity, and the Q4_0/Q8 activation fastpath dispatch checks.

### Features
- **CPU Forward-Pass Graph Tracer (GEO-XRAY-3)** — Added `src/cpu/forward_graph_trace.rs` with `ForwardGraphRecorder`, a self-contained JSONL tracer for CPU inference. It records per-token input embeddings, queries, keys, values, attention outputs, MLP hidden states, logits, and a confidence node, plus attention edges with per-head weights. Two new CLI flags expose it: `--forward-graph-trace <path>` and `--expected-attention <json>`. The meta record now includes `predicted_token` and `confidence`. The output is consumed by `geographdb-core/examples/plot_forward_graph.py` to render a transformer x-ray comparing actual vs expected attention positions. Verified on Qwen2.5-0.5B-instruct CPU greedy decode.
...
### Features
- **Branch Scoring and Divergence Metrics (Step 3)** — Added `CpuOpNode::Score`, a reduction operator that writes a scalar score into the arena. `ScoreMetric` supports `CosineSimilarity`, `L2Similarity`, `MeanActivation`, `NegEntropy`, and `CrossEntropy`. `CaptureContext::score_against()` records a score for the current timestamp without replay, and `GraphMap::branch_scores()`/`divergence()` expose the resulting branch ranking. The score log is persisted alongside the arena in the `GraphMap` sidecar.
- **Introspection Prompt Interface (Step 4)** — Added `src/cpu/graph/introspection.rs` with `GraphSummarizer`, `GraphSummary`, `IntrospectionPrompt`, and `IntrospectionReport`. A `GraphMap` is compressed into a short text prompt plus a fixed-size vector encoding; the model's structured response (`CHOICE:` / `REASON:`) is parsed back into a branch selection.
- **Feedback Loop as Graph Annotations (Step 5)** — Added `BranchAnnotation` (timestamp, bias, optional `IntrospectionReport`, optional semantic key) to both `CaptureContext` and `GraphMap`. Bias annotations are persisted in the `GraphMap` sidecar and can be looked up by timestamp or by key (`branch_biases`, `branch_bias_by_key`, `biases_by_key`). `CaptureContext::annotate_branch()` lets a session attach a bias at capture time.
- **Trace Dataset for Training (Step 6)** — Added `src/cpu/graph/dataset.rs` with `GraphTraceDataset`. It loads one or more persisted `GraphMap`s and emits `ProcessSupervisionExample`, `RejectionSamplingExample`, and `PreferencePair` records. A `score_index()` helper verifies lossless conversion back to the original branch scores.
- **Lightweight Mistake-Driven Adapter (Step 7)** — Added `src/cpu/graph/adapter.rs` with `BranchAdapter`, a tiny two-layer MLP trained on `GraphTraceDataset` preference pairs using a pairwise hinge loss. It scores per-branch feature vectors derived from `GraphSummarizer` and ranks branches via `rank_branches()`.
- **Open-Weight Experiments (Step 8)** — Added `src/cpu/graph/value_head.rs` with `BranchValueHead` (trainable linear layer on frozen base-model hidden states), `BranchChoiceHead` (linear multi-class classifier for full-prompt hidden states), `BranchLogitScorer` (scalar rescaling of "yes"/"no" answer logits), and `BranchLabelBias` (per-letter bias added to multi-branch choice logits). All are trained on preference pairs from the trace dataset. `src/cpu/graph/open_weight.rs` wires these trainers to the existing CPU inference path (`cpu_prefill`) and provides token-id lookup helpers.
- **Real-Session GraphMap Capture and Reload** — Added `cpu_full_forward_with_ctx` in `src/cpu/forward.rs` and `run_cpu_decode_loop_with_ctx` in `src/app/cpu_decode.rs` so a real CPU inference session can run through a `CaptureContext`. Each generated token becomes a timestamp/branch and is scored with `ScoreMetric::NegEntropy`. The CLI gained `--graph-map-dir`, `--load-graph-map-dir`, and `--graph-score-metric` flags. `src/app/cpu_inference.rs` persists the resulting `GraphMap` after generation and loads a previous session's map before generation, closing the capture → persist → reload loop.
- **GPU Decode GraphMap Trace Capture** — Extended the same `--graph-map-dir` / `--load-graph-map-dir` flags to the GPU inference path (`src/app/gpu_inference.rs`). When capturing, the GPU greedy fastpath is disabled for decode so logits are downloaded to the host; each generated token is recorded as a `GpuTraceEntry` (timestamp, position, input token, sampled token, score). The trace is stored in a new `gpu_trace` section of the existing `GraphMap` sidecar, so GPU sessions on larger models can produce persistent reasoning traces without copying full activation tensors.
- **Online Value-Head Reranker (Step 11)** — Added `--value-head-path`, `--rerank-top-k`, `--rerank-scale`, `--train-value-head-from-traces`, and `--save-value-head` CLI flags. During CPU decode, the top-k candidate next tokens are scored by a loaded `BranchValueHead` through speculative forward passes (KV cache clone + one decode step per candidate), and the value-head score is added as a logit bias before sampling. A training utility converts persisted `GraphMap` traces into `BranchValueExample`s and trains the head with MSE. This closes the capture → train → rerank loop. Verified on the 0.5B model: top-5 rerank adds ~5.9× latency per token on CPU (37 ms → 220 ms), and the reranked trajectory differs from greedy.
- **Speculative Candidate Branch Recording (Step 11.5/B)** — The online reranker now persists every evaluated top-k candidate as a `CandidateBranch` entry inside the `GraphMap` sidecar. Each entry records parent timestamp, token id, value-head score, biased logit, and whether the candidate was selected. The sidecar section is optional and backward-compatible; old `GraphMap`s load with an empty candidate list. This preserves the reranker search tree for inspection, process-supervision training, and future beam-search extensions.
- **Reranker Chosen-State Reuse (Step 11.5/C)** — The online reranker now reuses the chosen candidate's speculative forward state instead of recomputing it on the next real decode step. A single `kv_scratch` is shared across all candidate evaluations, and only small per-candidate KV deltas (K/V slices at the candidate position, plus conv-state when needed) are captured. After sampling, the chosen delta is applied to a fresh KV clone and stashed as a `ReusableState` for the next iteration, eliminating one full forward pass per generated token. On the 0.5B Qwen model with top-5 reranking, measured overhead dropped from ~5.9× to ~4.2× baseline CPU decode time.
- **Multi-Token Beam/Lookahead Reranking (Step 11.5/D)** — Added `--rerank-beam-depth <D>` to extend the value-head reranker from single-token scoring to a short deterministic greedy beam. For each top-k candidate, the model runs the candidate forward plus up to `D-1` greedy continuation tokens, summing the value-head scores along the way. The cumulative beam score is used to bias the original logits and is recorded in `CandidateBranch.value_score`. Depth `1` preserves the existing single-token behavior. On the 0.5B Qwen model, top-5 depth-2 reranking costs ~7.1× baseline CPU decode time and produces a different trajectory than depth-1.
- **Multi-Hypothesis Beam Search (Step 11.5/E)** — Added `--rerank-beam-width <B>`. When `B > 1` and a value head is loaded, the decode loop keeps `B` hypotheses alive, expands each by its top-k candidates each step, and prunes back to the top `B` expansions by mean cumulative beam score. Candidate branches for every evaluated expansion are recorded in the `GraphMap` sidecar, with `chosen = true` for expansions that survive pruning. The full winning sequence is printed after generation. On the 0.5B Qwen model, top-5 width-2 reranking costs ~7.9× baseline CPU decode time and chooses a different sequence than width-1.
...
### Fixed
- **Q5_0/Q5_1 GPU+CPU dequant nibble layout** — The four Q5_0/Q5_1 GPU HIP kernels (`q5_0_gemv`, `q5_0_gemm`, `q5_1_gemv`, `q5_1_gemm`) and the CPU `embed_q5_0`/`embed_q5_1` dequant functions decoded the 5-bit nibbles with the wrong packing: they read byte `i` as elements `2i`/`2i+1` instead of the GGUF/llama.cpp layout where byte `i` holds element `i` (low nibble) and element `i+16` (high nibble) — identical to Q4_0. This caused the Q4_K_M layer-0 GPU forward to diverge from the CPU reference by ~0.96 in the hidden state (Qwen2.5-0.5B-instruct-q4_k_m). The isolated Q5_0/Q5_1 GEMV tests passed only because the CPU `embed_q5_*` reference shared the same wrong layout (circular validation). Fixed all six sites to the `(i, i+16)` layout; the GPU Q4_K_M layer-0 divergence dropped from 0.962349 → 0.000003. Verified: all Q4_K/Q5_0/Q5_1/Q5_K/Q6_K/Q8_0 GEMV/embed tests pass; Q6_K end-to-end inference produces coherent output with no GPU reset.
- **Forward graph `predicted_token` / `confidence` alignment** — Moved `ForwardGraphRecorder::record_confidence` to the top of the decode loop so the recorded token and probability correspond to the token actually emitted, not the next token sampled for the following step. This fixes meta records where `predicted_token` pointed to the following token and `confidence` was computed against the wrong distribution.
- **`Tokenizer` trait import in `cpu/graph/open_weight.rs`** — Replaced the unused `BpeTokenizer` concrete import with `crate::tokenizer::Tokenizer` so the `&dyn Tokenizer` parameters in `extract_hidden_state` and related helpers resolve under `cargo check --features cpu-graph`.
- **`Tokenizer` trait usage in `test_cpu_graph_train_value_head.rs`** — Updated `generate_completion` to accept `&dyn Tokenizer` so the test compiles with `TokenizerHandle` returned by `ModelFile::tokenizer()`.
...
- **Gemma4 text attention scale** — `configure_gemma4` now sets `attention_scale` to `1.0`, matching the Hugging Face implementation where QK norm replaces the `1/sqrt(head_dim)` scaling. The previous value caused attention scores to be scaled incorrectly.
...
- **`apply_qk_norm` value-norm support** — Extended `apply_qk_norm` to optionally apply a scaleless RMS norm to the value tensor, matching Gemma4's `v_norm`.
- **Q4_1 × Q8_0 AVX2 kernel correctness** — Fixed the horizontal reduction that computes `sum(q8)` for the asymmetric Q4_1 min-offset correction. The old implementation summed only half of the block and produced wildly wrong offsets, which was the dominant cause of Gemma4 garbage output on CPUs with AVX2.
- **Q4_0 × Q8_0 AVX-VNNI kernel correctness** — Replaced the incorrect `_mm256_dpwssd_avx_epi32` (16-bit multiply-add) path with the proper `_mm256_dpbusd_avx_epi32` (unsigned×signed byte dot) plus the signed-offset correction for Q4_0's `-8..7` nibble range.
- **SentencePiece tokenizer dispatch** — `ModelFile::tokenizer()` now routes `gemma4`, `spm`, `llama`, and `t5` tokenizer model names to the new SentencePiece implementation, enabling correct tokenization for Gemma4 checkpoints.
- **SentencePiece placeholder-score fallback** — `SpmTokenizer` now detects GGUF `tokenizer.ggml.scores` arrays that are uniform or sequential placeholders (e.g. `0.0, 1.0, 2.0, ...`) and switches to greedy longest-match segmentation instead of score-driven Viterbi. This fixes Gemma4 encoding falling back to single-byte/character tokens.
- **Gemma4 token embedding scale** — `configure_gemma4` now sets `embedding_scale = 1.0` because the GGUF `token_embd.weight` already has the `sqrt(hidden_size)` scaling baked in by the converter. The previous extra scaling made embeddings ~39× too large and produced wrong logits.
- **Gemma4 PLE projection norm ordering** — `cpu_compute_ple_inputs` now RMS-normalizes the per-layer projection *before* adding the per-layer token embeddings, matching the Hugging Face `project_per_layer_inputs` implementation. The old code normalized the sum, giving incorrect per-layer inputs.
- **BF16 GEMV support** — Added `gemv_bf16` / `gemv_bf16_transposed` in `src/cpu/ops/gemv/dense.rs` and wired them into `dispatch_gemv`. This fixes CPU panic when `per_layer_model_proj.weight` is stored as BF16 (seen in some Gemma4 Q4_0 conversions).
...
- **CPU Graph Branch Scoring Validation Experiment (Step 3)** — Added `tests/test_cpu_graph_branch_scoring.rs`. It proves that a `Score` node replayed through `execute_window` matches the expected cosine similarity, that `CaptureContext::score_against()` ranks a branch moved toward a target above a branch moved away from it, and that `GraphMap` round-trips the score log.
- **CPU Graph Introspection Validation Experiment (Step 4)** — Added `tests/test_cpu_graph_introspection.rs`. It builds `GraphMap` branch pairs with known scores, uses `GraphSummarizer` to prompt the local Qwen2.5-0.5B-instruct model, parses the `IntrospectionReport`, and verifies the model selects the higher-scoring branch more often than random. The model-based test is marked `#[ignore]` because it loads the 0.5B GGUF and runs CPU inference for ~10 minutes.
- **CPU Graph Feedback-Loop Validation Experiment (Step 5)** — Added `tests/test_cpu_graph_feedback_loop.rs`. A first session scores four branches, stores each score as a `BranchAnnotation` bias in the `GraphMap`, and persists it. A second session loads those biases by key and evaluates branches in bias-descending order, reaching the best branch in 1 evaluation versus 3 in the default order.
- **CPU Graph Trace-Dataset Validation Experiment (Step 6)** — Added `tests/test_cpu_graph_trace_dataset.rs`. It builds several `GraphMap`s, persists them into a directory, loads them back with `GraphTraceDataset::from_dir`, and verifies that process-supervision, rejection-sampling, and preference-pair exports preserve every `(trace_id, timestamp, score)` triple without loss.
- **CPU Graph Adapter Validation Experiment (Step 7)** — Added `tests/test_cpu_graph_adapter.rs`. It generates 24 training traces and 12 held-out test traces, trains a `BranchAdapter` on preference pairs, and verifies the adapter picks the highest-scoring branch on 12/12 test traces versus 4/12 for a random baseline.
- **CPU Graph Value-Head Validation Experiment (Step 8)** — Added `tests/test_cpu_graph_value_head.rs`, a fast synthetic unit test that proves `BranchValueHead` learns to rank hidden vectors above a random baseline.
- **CPU Graph Open-Weight Validation Experiment (Step 8)** — Added `tests/test_cpu_graph_open_weight.rs`. It loads the local Qwen2.5-0.5B-instruct GGUF, applies the chat template to the standard two-branch introspection prompt, extracts the final hidden-state vector, and trains a `BranchChoiceHead` (linear binary classifier) on frozen base-model representations. On held-out semantic branch pairs the trained head reaches **8/8** accuracy versus **4/8** for a random baseline. The test is marked `#[ignore]` because it runs CPU inference on the 0.5B model dozens of times.
- **CPU Graph Real-Session Capture/Reload Validation** — Added `tests/test_cpu_graph_real_session.rs`. It runs the `rocmforge` CLI with `--graph-map-dir`, verifies the persisted `GraphMap` files, then runs a second invocation with `--load-graph-map-dir` and asserts the previous session is loaded and a new map is saved. Marked `#[ignore]` because it loads the 0.5B model and runs real CPU inference.
- **GPU Graph Trace Capture/Reload Validation** — Added `tests/test_gpu_graph_trace.rs`. It runs the `rocmforge` CLI with `--gpu --graph-map-dir`, loads the saved `GraphMap`, verifies the `gpu_trace` and `score_log` have matching entries, then reloads the map in a second invocation and verifies it is saved again. Marked `#[ignore]` because it requires an AMD GPU and loads the 0.5B model.
- **Online Value-Head Reranker Validation** — Added `tests/test_cpu_graph_online_reranker.rs`. It constructs a deterministic `BranchValueHead` (score = first hidden dimension), saves it, runs greedy CPU decode with and without `--value-head-path`, and asserts the reranker evaluates top-k candidates. The test also loads the saved `GraphMap` and verifies that `CandidateBranch` entries were persisted and at least one was marked chosen. Latency per token is printed for both runs; on the 0.5B model top-5 rerank costs ~5.9× the baseline CPU decode time.
...
- **Gemma4 SentencePiece tokenizer parity test** — Added `tests/spm_encode.rs` which loads the Ollama `gemma4:e2b` GGUF and asserts exact token-ID matches against the Hugging Face tokenizer for several English phrases, special tokens, and space-prefixed inputs.
...
### Added
- **GitHub Actions CI Pipeline**: Created `.github/workflows/ci.yml` based on the grounded coding verification gate. It includes parallel jobs for Documentation & License validation, CPU-only build/test, GPU (ROCm) build check within an official `rocm/dev-ubuntu-22.04` container, and comprehensive security scans (`cargo audit`, `cargo deny`, `gitleaks`, and `semgrep`).
...
### Fixed
- **Rust 1.95 Clippy Compatibility** — Fixed ~60 pre-existing clippy errors introduced by Rust 1.95 new lints across 17 files. Replaced `needless_range_loop` with iterator patterns, `manual_clamp` with `clamp`, added `reason = "..."` to all `#[allow(...)]` attributes, fixed `doc_lazy_continuation`, `too_many_arguments`, `missing_safety_doc`, `manual_memcpy`, `type_complexity`, `sliced_string_as_bytes`, `approx_constant`, `assertions_on_constants`, `unused_unsafe`, and `explicit_counter_loop` warnings. All `#[allow(...)]` attributes now comply with the `M-LINT-OVERRIDE-EXPECT` pre-commit hook guideline requiring `reason = "..."`.
