...
- **Online Value-Head Reranker Validation** — Added `tests/test_cpu_graph_online_reranker.rs`. It constructs a deterministic `BranchValueHead` (score = first hidden dimension), saves it, runs greedy CPU decode with and without `--value-head-path`, and asserts the reranker evaluates top-k candidates. Latency per token is printed for both runs; on the 0.5B model top-5 rerank costs ~5.9× the baseline CPU decode time.
