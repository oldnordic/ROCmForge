...
- **CPU Graph Open-Weight Validation Experiment (Step 8)** — Added `tests/test_cpu_graph_open_weight.rs`. It loads the local Qwen2.5-0.5B-instruct GGUF, presents multi-branch choice prompts, extracts per-label logits, trains a `BranchLabelBias`, and reports biased/unbiased/random accuracies on held-out synthetic traces. The test is marked `#[ignore]` because it runs CPU inference on the 0.5B model dozens of times.
