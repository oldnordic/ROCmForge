...
fn common_header_exists() {
    ...
    let content = std::fs::read_to_string(header).expect("Failed to read common.hip");

    assert!(content.contains("CHECK_HIP"), "Must define CHECK_HIP macro");
    assert!(
        content.contains("CHECK_BOUNDS"),
        "Must define CHECK_BOUNDS macro"
    );
    assert!(
        content.contains("CHECK_LAST"),
        "Must define CHECK_LAST macro"
    );
    assert!(content.contains("QK_K"), "Must define QK_K constant");
    assert!(
        content.contains("BLOCK_SIZE"),
        "Must define BLOCK_SIZE constant"
    );
}
...
fn test_kernel_exists() {
    ...
    let content = std::fs::read_to_string(kernel).expect("Failed to read test_kernel.hip");

    assert!(
        content.contains("vector_add"),
        "Must define vector_add kernel"
    );
    assert!(
        content.contains("test_vector_add"),
        "Must define FFI wrapper"
    );
}
...
fn build_directory_exists_or_creatable() {
    ...
    if build_dir.exists() {
        ...
        assert!(
            build_dir.exists(),
            "build directory should exist after creation"
        );
    }
}
    ...
    match build_result {
        Ok(output) => {
            if output.status.success() {
                ...
                let metadata =
                    std::fs::metadata(lib_path).expect("Failed to read library metadata");
...
fn buildrs_invokes_cmake() {
    ...
    let content = std::fs::read_to_string(buildrs).expect("Failed to read build.rs");

    // Verify compile_quant_kernels is called
    assert!(
        content.contains("compile_quant_kernels()"),
