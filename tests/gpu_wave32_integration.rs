...
fn test_wave32_device_properties() {
    ...
    assert!(
        stats.safely_allocatable_gb() >= 0.0,
        "Should have non-negative safe VRAM"
    );
...
fn test_wave32_kernel_launch_no_vram_leak() {
    ...
    // Verify that GPU device initialization, stream allocation, and VRAM tracking
    // do not leak any VRAM across active sessions.

    drop(device);
    let after = GpuDevice::init(0).expect("Failed to re-init GPU");
    let after_stats = after.vram_stats().expect("Failed to get VRAM after");

    // Allow 40 MB tolerance for HIP driver context caching and pooling overhead
    let leaked_mb = (before.used_vram as i64 - after_stats.used_vram as i64).abs() / (1024 * 1024);
    assert!(leaked_mb <= 40, "VRAM leak detected: {} MB", leaked_mb);
}
