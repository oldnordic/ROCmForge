...
fn test_scratch_memory_usage() {
    ...
    if let Ok(scratch) = scratch {
        ...
        let expected_bytes =
            (h + q + kv + kv + q + h + ff + ff + v) * 4; // * 4 for f32
...
fn test_scratch_all_buffers_non_empty() {
    ...
    if let Ok(scratch) = scratch {
        ...
        assert!(!scratch.normed.is_empty(), "normed buffer should not be empty");
        ...
        assert!(!scratch.attn_out.is_empty(), "attn_out buffer should not be empty");
        assert!(!scratch.layer_out.is_empty(), "layer_out buffer should not be empty");
        assert!(!scratch.gate.is_empty(), "gate buffer should not be empty");
        assert!(!scratch.swiglu.is_empty(), "swiglu buffer should not be empty");
        assert!(!scratch.logits.is_empty(), "logits buffer should not be empty");
    }
}
