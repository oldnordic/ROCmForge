...
- [ ] **Step 4: Add Q4_K case to dispatch_gemm**

```rust
// src/cpu/ops.rs

// In dispatch_gemm function, inside the match meta.wtype block:
GgmlType::Q4_K => {
    // GEMM uses same signature for all types
    // For now, use AVX2 when available
    gemm_q4_k_q8_k_dispatch(w, x, y, out_dim, in_dim);
    Ok(())
}
```

Note: `dispatch_gemm` returns `Result<(), CpuError>` so we wrap with `Ok(())`.

Note: For now, we use the same function signature. The GEMM wrapper will be added in the next task.
...
git commit -m "feat(ops): integrate Q4_K × Q8_K into dispatch

- Add Q4_K case to dispatch_gemv and dispatch_gemm
- Export kernel functions from cpu module
- Enable runtime CPU feature detection
- Support both AVX2 and scalar fallback
...
- [ ] **Step 4: Update dispatch_gemm to use new signature**
...
GgmlType::Q4_K => {
    if meta.needs_transpose {
        ...
        gemm_q4_k_q8_k_scalar(w, x, y, m, n);
    } else {
        gemm_q4_k_q8_k_dispatch(w, x, y, m, n);
    }
}
```
